...
AS
   /******************************************************************************
       ...
       1.3        14-Jun-2019      Ardianto            1. Menambahkan Validasi untuk merubah status Need Halal dan Need MD/Halal
      ******************************************************************************/
   ...
   */
   IS
      ...
      v_reference_item_fg              VARCHAR2 (40);  -- Update by Ardi on 21 May 2019
...
--                  END;
                  ...
                  -- Update by Ardi on 21 May 2019
                  IF (v_template = 'FGSA MAKE' OR v_template = 'FGSA BUY') THEN
                    v_reference_item_fg := v_mst_item_reg.item_code;
                  ELSE
                    v_reference_item_fg := v_mst_item_reg.reference_item_fg_id;
                  END IF;
                  -- Update by Ardi on 21 May 2019
                  
                  --logf('Reference item code '||v_reference_item_fg);
                  ...
                  INSERT INTO mtl_system_items_interface (set_process_id,
                          ...
                          VALUES (
                                    ...
                                    v_reference_item_fg, --v_mst_item_reg.reference_item_fg_id, -- Update by Ardi on 21 May 2019
              ...
              --logf('Reference item code '||v_reference_item_fg);
               ...
               INSERT INTO mtl_system_items_interface (set_process_id,
                       ...
                       VALUES (
                                 ...
                                 v_mst_item_reg.buffer_packaging,
                                 v_mst_item_reg.item_trial_id,
                                 v_reference_item_fg, --custd.reference_item_fg_id, -- Update by Ardi on 21 May 2019
                                 v_mst_item_reg.weight_factor,
                                 v_mst_item_reg.shelf_life,
                                 NULL,
                                 --PARENT_ITEM_KN
...
--               END;
              ...
              IF (v_template = 'FGSA MAKE' OR v_template = 'FGSA BUY') THEN
                v_reference_item_fg := v_mst_item_reg.item_code;
              ELSE
                v_reference_item_fg := ctf.reference_item_fg_id;
              END IF;
              -- Update by Ardi on 21 May 2019
              ...
              --logf('Reference item code '||v_reference_item_fg);
               ...
               INSERT INTO mtl_system_items_interface (set_process_id,
                       ...
                       VALUES (
                                 ...
                                 v_mst_item_reg.buffer_packaging,
                                 v_mst_item_reg.item_trial_id,
                                 v_reference_item_fg, --ctf.reference_item_fg_id, -- Update by Ardi on 21 May 2019
                                 v_mst_item_reg.weight_factor,
                                 v_mst_item_reg.shelf_life,
                                 NULL,
                                 --PARENT_ITEM_KN
      ...
      THEN
         ...
         UPDATE xxshp_inv_master_item_reg
          ...
          -- Added Fajrin 2018-07-14
            ...
            -- Added Ardi 2019-06-14 Need Halal
            IF v_template_name IN ('FGSA BUY','FGSA MAKE','PM') THEN
            
                    select count(halal_certificate_num), count(akasia_num)  into v_counthalal, v_countakasia
                    from xxshp_inv_manufacturers xim, xxshp_inv_mfg_part_numbers mpn
                    where xim.mfg_id = mpn.mfg_id
                    and reg_hdr_id = p_reg_hdr_id;
                    
                    logf ('v_counthalal : ' || v_counthalal );
                    logf ('v_countakasia : ' || v_countakasia );
                ...
                logf ('v_template_name in PM : ' || v_template_name );
                    /*IF v_countakasia > 0 THEN
                            IF v_counthalal = 0 THEN
                                change_status(v_item_id, 84, 'Need Halal');
                                UPDATE xxshp_inv_master_item_reg SET need_halal = 'Y' WHERE reg_hdr_id = p_reg_hdr_id; COMMIT;
                                logf ('end change status in PM');
                            END IF;
                    END IF;*/     
                ELSE
                    ...
                    IF v_lob IN ('KNG','KN1','KN2','KN3','KN4','KFA') THEN --and v_tipe = '2A000' THEN -- comment by ardi 1 Jul 2019
                            IF v_counthalal = 0 THEN
                                change_status(v_item_id, 84, 'Need Halal');
                                UPDATE xxshp_inv_master_item_reg SET need_halal = 'Y' WHERE reg_hdr_id = p_reg_hdr_id; COMMIT;
                                logf ('end change status in FG');
                ...
                END IF;
            END IF;
            --- end Added Ardi 2019-06-14 Need Halal
            
            -- Added Ardi 2019-06-14 Need MD dan Halal
            IF v_template_name IN ('FGSA BUY','FGSA MAKE','PM') THEN
            
                    select count(halal_certificate_num), count(certificate_md_num), count(akasia_num) 
                    into v_counthalal, v_countmd, v_countakasia
                    from xxshp_inv_manufacturers xim, xxshp_inv_mfg_part_numbers mpn
                    where xim.mfg_id = mpn.mfg_id
                    and reg_hdr_id = p_reg_hdr_id;
                ...
                logf ('v_template_name in PM : ' || v_template_name );
                    IF v_countakasia > 0 THEN
                        IF v_countmd = 0  AND v_counthalal = 0 THEN
                            change_status(v_item_id, 84, 'Need MD/Ha');
                            UPDATE xxshp_inv_master_item_reg SET need_md = 'Y', need_halal = 'Y' WHERE reg_hdr_id = p_reg_hdr_id; COMMIT;
                            logf ('end change status in PM');
                        END IF;
                    END IF;     
                ELSE
                    ...
                    IF v_lob IN ('KNG','KN1','KN2','KN3','KN4') THEN --and v_tipe = '2A000' THEN
                            IF v_countmd = 0 AND v_counthalal = 0 THEN
                                change_status(v_item_id, 84, 'Need MD/Ha');
                                UPDATE xxshp_inv_master_item_reg SET need_md = 'Y', need_halal = 'Y' WHERE reg_hdr_id = p_reg_hdr_id; COMMIT;
                                logf ('end change status in FG');
                ...
                END IF;
            END IF;
            --- end Added Ardi 2019-06-14 Need MD dan Halal
