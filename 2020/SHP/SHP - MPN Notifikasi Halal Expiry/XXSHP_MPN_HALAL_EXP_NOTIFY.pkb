...
as
   ...
   is
      p_to                       varchar2 (2000) := 'reza.fajrin@kalbenutritionals.com';--'ardianto.ardi@kalbenutritionals.com';-- 
      p_cc                       varchar2 (2000) := 'Evi.Rachmaniatun@kalbenutritionals.com,Debby.Ardi@kalbenutritionals.com,ardianto.ardi@kalbenutritionals.com';
      p_bcc                      varchar2 (2000);
   ...
   begin
      ...
      begin
         ...
         utl_smtp.write_data (
            v_connection,
               utl_tcp.crlf||'Please be aware that attached Material Halal Expiry Date would shortly expired in ' || to_char(add_months(sysdate,4),'MON-YYYY')||chr(13)||chr(10)||
         ...
         v_clob :=
               'Category,Item Code,Item Desc,Organization Code,Supplier Name,Part,Sertificate Halal Number,Expiry Date,Phase Out,Type Email,Item Template, Halal Body, Manufacturer Name'
            || utl_tcp.crlf;
   ...
   begin
      ...
      begin
         ...
         utl_smtp.write_data (
            v_connection,
               utl_tcp.crlf||'Please be aware that attached Material Halal Expiry Date would shortly expired in ' || to_char(add_months(sysdate,4),'MON-YYYY')||chr(13)||chr(10)||
               'These item status would be change to PHASE OUT in ' || to_char(add_months(add_months(sysdate,4),-3),'MON-YYYY')||'.'||chr(13)||chr(10)||
               'All PR (Purchase Requisition), PO (Purchase Order), BO (Batch Order) using these items can not be created when the item status is in phase out.'
            || utl_tcp.crlf);
         utl_smtp.write_data (
            v_connection,
               utl_tcp.crlf||'Please renew the Halal Expiry Date to avoid Status Changes to Phase Out.'
            || utl_tcp.crlf);
         utl_smtp.write_data (
            v_connection,
               utl_tcp.crlf||'NOTE - Please do not reply since this is an automatically generated e-mail.'
