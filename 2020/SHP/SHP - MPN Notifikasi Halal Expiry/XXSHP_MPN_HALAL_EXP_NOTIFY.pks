...
AS
   ...
   PROCEDURE send_mail_m4 (p_total IN NUMBER, p_result OUT VARCHAR2);
   ...
   IS
      SELECT DISTINCT
       ...
       WHERE     mmp.manufacturer_id = mm.manufacturer_id
             ...
             AND TRUNC (TO_DATE (mmp.attribute10, 'yyyy/mm/dd hh24:mi:ss')) BETWEEN TRUNC (
                                                                                       ADD_MONTHS (
                                                                                          SYSDATE,
                                                                                          3))
                                                                                AND TRUNC (
   ...
   IS
      SELECT DISTINCT
       ...
       WHERE     mmp.manufacturer_id = mm.manufacturer_id
             ...
             AND mcb.category_id = mic.category_id
             AND category_set_id = '1100000042'
             AND TRUNC (TO_DATE (mmp.attribute10, 'yyyy/mm/dd hh24:mi:ss')) <=
   ...
   IS
        SELECT DISTINCT kn,
                        ...
                        item_template
          FROM xxshp_mpn_halal_exp_stag
         WHERE TRUNC (creation_date) = TRUNC (SYSDATE) AND type_email = 'M-4'
      ORDER BY item_code;
