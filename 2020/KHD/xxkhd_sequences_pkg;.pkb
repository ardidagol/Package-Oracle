CREATE OR REPLACE PACKAGE BODY APPS.xxkhd_sequences_pkg
/* $Header: xxkhd_sequences_pkg.pkb 122.5.1.5 2016/03/01 15:29:00  Edi Yanto $ */
AS
   /**************************************************************************************************
         NAME: xxkhd_sequences_pkg
         PURPOSE:
   ...
   */
   IS
      v_seq_type_id   xxkhd_sequences.seq_type_id%TYPE   := 0;
      v_last_val      xxkhd_sequences.LAST_VALUE%TYPE    := 0;
   ...
   BEGIN
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
            -- select sequence from xxkhd_sequences_s
            SELECT xxkhd_sequences_s.NEXTVAL
              INTO v_seq_type_id
              FROM DUAL;
   ...
   */
   IS
      ...
      v_seq_type   xxkhd_sequences.seq_type%TYPE;
   BEGIN
      -- call get_lastvalue
